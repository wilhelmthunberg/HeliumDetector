...
os.chdir('/Users/w/Project_AppliedPhys/Code/results')

def dose(cr):
    return 5.32 * (cr/1166650)
"""
...
for volt in ['1100', '1200', '1300', '1400', '1500']:
    peak_width = 10             ## required to use the scipy peak finder and not trigger on rising-edge noise


    # Read the neutron pulse train:
    df_n = pd.read_csv('SF/tau=6.409214453193086e-06/count_rate=15925')
    ...
    gamma_rates = ['1179947', '11799467','23598933', '35398400','47197867','58997333' ]
    ...
    for i in range(N_gamma_rates):
        df_g[i] = pd.read_csv('gamma/tau=6.409214453193086e-06/count_rate=' + gamma_rates[i]) 
    ...
    n, bins, patches = ax[0].hist(peak_V_n_flat_list, bins=200, range=[0, 10], density=True, facecolor='r', alpha=0.75, label='SF neutrons')
    ax[0].set_yscale('log')
    ax[0].text(0.9, 0.5,'Neutrons', horizontalalignment='center', verticalalignment='center', transform = ax[0].transAxes)


    residual_efficency = np.zeros(N_gamma_rates)
    rad_h = np.zeros(N_gamma_rates) 

    for i in range(N_gamma_rates):
        n, bins, patches = ax[1+i].hist(peak_V_g_flat_list[i], bins=200, range=[0, 10], density=True, facecolor='g', alpha=0.75, label='Gammas, rate ' + str(i))
        n, bins, patches = ax[1+i].hist(V_dist_g_flat_list[i], bins=200, range=[0, 10], density=True, facecolor='y', alpha=0.75, label= '"All voltages"')
        rad_h[i] = dose(int(gamma_rates[i]))
        residual_efficency[i] = round(100*len(np.where(np.array(peak_V_n_flat_list)>max(peak_V_g_flat_list[i]))[0])/len(peak_V_n_flat_list),1); 
        ax[1+i].set_yscale('log')
        ax[1+i].text(0.78, 0.5,'Gammas (dose: ' + "{:.2f}".format(float(rad_h[i])) + 'rad/h )', horizontalalignment='center', verticalalignment='center', transform = ax[1+i].transAxes)



    plt.xlabel('Pulse height [a. u.]')
    f.text(0.02, 0.5, 'Counts per bin (normalised)', va='center', rotation='vertical')
    ...
    plt.ylabel('Counts per bin (normalised)')
    ...
    gr = np.array([1179947, 11799467,23598933, 35398400,47197867,58997333])/1e6
    fig = plt.figure()
    plt.plot(rad_h,residual_efficency , linestyle='-', marker = 'o',color='blue', linewidth=2)
    plt.title('Residual efficency')
    plt.xlabel('Dose [rad/h]')
