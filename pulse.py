...
def pulse(det,mu_e,mu_i,x,y,z,dr,q=1.60217662 *10**-19,t_start=0):
  """
...
Is,sigs,ts = pulse(det, det.Mob, det.mu_i, det.r_c/2, det.r_c/2, det.r_c/1.1, 1e-5)

fig, ax1 = plt.subplots()
ax2 = ax1.twinx()

ax1.plot(np.array(ts)*1e6, sigs,'g-')
ax2.plot(np.array(ts)*1e6, Is,'b-')
ax1.set_xlabel('Time '+r'[$\mu s$]')
ax1.set_ylabel('Output voltage [V]', color='green')
ax2.set_ylabel('Induced Current [A]', color = 'blue')

plt.title('Pulse genrated by a single charge.')
plt.show()
"""